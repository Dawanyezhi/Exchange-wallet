...
6. 每条链调研都必须提供“交易构建与签名生产案例”：至少覆盖一笔提现或归集交易，从业务请求、链上参数、unsigned tx、签名服务入参/出参、签名前策略校验、签后反解析验签、raw tx 广播、落库状态变更到失败/重发处理。不得只写抽象流程或“签 32 字节 hash”。
7. 给出可落地结论：是否适合交易所接入、最小上线能力、不建议首期支持的能力、生产风险点、对当前项目的改造建议。
...
每份链调研必须包含可执行思路明确的交易构建和签名生产案例，并按链模型写出关键差异：

- Account/Nonce 链：chain id/network id、nonce 获取和锁定、gas/fee cap、to/value/data、Token 合约调用、签名哈希、防重放、签后 raw tx 反解析。
- UTXO 链：选币、输入锁定、找零、dust、fee rate、每个 input 的 previous output 金额和脚本、sighash、witness/scriptSig、txid/wtxid、RBF/CPFP。
- Memo/Tag 链：目标地址、memo/tag 字段、tag 归属校验、漏填/错填处理、签名前后 memo/tag 反校验。
- 多资产/合约链：Token/NFT 标准、合约地址白名单、事件/日志索引、授权/approve 风险、失败交易过滤。
