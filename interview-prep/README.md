...
我们做的是一套交易所托管钱包系统，整体分三层。最底层是密钥管理层 walletmanager，负责密钥的生成、加密存储和安全分发，钱包服务的私钥永远不在运行时暴露。中间层是 irwallet，这是一个通用钱包引擎，把 UTXO、账户型、Tag 型三种链的核心逻辑全部抽象进去了，充值、提现、归集、对账这些能力都在这一层实现。最上层是 ethfork，专门处理 EVM 多链，目前支持 60+ 条链，BSC、Polygon、ETH、Arbitrum、Optimism、zkSync、Taiko 这些全在里面跑。

日常业务主要是四件事：充值（链上扫块确认后通知交易所上账）、提现（交易所下单我们签名广播）、归集（把散落在充值地址的资金聚到热钱包）、对账（每 2 秒对比链上余额和本地余额，发现偏差立刻报警）。

核心设计理念是：**安全优先、分层隔离、配置驱动**。密钥和钱包逻辑完全分离，每条链的确认数、Gas 策略、特性开关独立配置，新增一条链基本不需要改代码。
...
交易所
  ...
  ├─ 提现下单 ──────────────────────────────────────────────┐
  │                                                         │
  └─ 充值通知 ←─── MQ ←─── reconciler（对账确认）            │
                                                           ▼
               ┌──────── irwallet 通用钱包引擎 ────────────┐
               │                                          │
               │  syncer（同步）                           │
               │    │                                     │
               │    ├──→ reconciler（对账）──→ MQ 通知交易所│
               │    │                                     │
               │    └──→ firefly（热钱包管理）              │
               │              │                           │
               │              └──→ 签名请求 ──────────────┼──┐
               ...
               ┌──── walletmanager 密钥管理层 ────────────────┐
               │                                             │
               │  ① Ed25519 签名鉴权（时间窗口防重放）          │
               │         │                                   │
               │  ② BIP32 派生子密钥（币种 FNV-1a 索引）       │
               │         │                                   │
               │  ③ X25519 ECDH 密钥交换                     │
               │         │                                   │
               │  ④ AES-256-GCM 加密传输子私钥               │
